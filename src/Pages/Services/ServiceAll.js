...
const ServiceAll = () => {
  ...
  const [serviceAllLoader, setServiceAllLoader] = useState(false);
  const [allServices, setServices] = useState([]);

  useEffect(() => {
    setServiceAllLoader(true);
    fetch("https://weeding-photographer-server.vercel.app/services")
      .then((res) => res.json())
      .then((data) => {
        setServices(data);
        setServiceAllLoader(false);
      });
  }, []);

  if (loader || serviceAllLoader) {
    return (
      <Spinner style={{ margin: "300px 700px" }} className="" animation="border" role="status">
        ...
        <div className="w-100">
          <Row xs={1} md={3} className="g-4">
            {allServices.map((singleService) => (
              <Col key={singleService?._id} single_service={singleService}>
                <Card className="h-100">
                  ...
                  <Card.Body>
                    ...
                    <Card.Text>{singleService?.description.slice(0, 40)}...</Card.Text>
